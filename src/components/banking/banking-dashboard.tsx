...
export function BankingDashboard() {
  ...
  async function connectBank() {
    ...
    try {
      const result = await startBankLink({data: {institutionId: selectedInstitutionId}})
                ...
                <div key={account.id} className="flex items-center justify-between gap-4 rounded-md border p-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {isSyncing ? (
                        <span
                          data-testid={`bank-account-${account.id}-syncing`}
            ...
            transactions.map(transaction => (
              <div key={transaction.id} className="grid gap-1 rounded-md border p-3 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {accountNamesById.get(transaction.bankAccountId) ?? 'Unknown account'} ·{' '}
                    {transaction.bookingDate ?? transaction.valueDate ?? 'No date'} · {transaction.status}
                  </p>
                </div>
