...
- `apps/flue/` is the temporary Flue sidecar service for automated categorization workflows and their active-run trace during the migration. Ask Penge no longer uses Flue.
- `apps/eve/` owns the Eve finance assistant, the authorized default chat channel, and the internal categorization spike channel. Browser traffic always enters through `apps/web` authorization boundaries.
...
Ask Penge uses Eve's default conversation channel through the same-origin `/api/eve/chat/:chatId/eve/v1/session*` proxy. The browser addresses an app-owned chat id and never receives a direct Eve host, service bearer, or authority header. On every request, `apps/web` authenticates the Better Auth cookie, authorizes chat ownership and current team membership, validates the exact route/body/origin, serializes turns with database compare-and-set state, and mints a short-lived capability limited to `{purpose: 'chat-session', userId, teamId, chatId}`. The Eve channel verifies that capability and tools read scope only from Eve session auth.
...
Flue remains a separate Node-target service only for the existing automated categorization workflow and active-run trace until migration Sections 5–6 land. The web workflow boundary validates team access, reserves app-visible state, and calls Flue with `PENGE_FLUE_BASE_URL` and `PENGE_FLUE_INTERNAL_TOKEN`; the browser can reach only authorized `GET`/`HEAD /api/flue/runs/:flueRunId` trace reads. There is no app-wide Flue provider and no browser-facing Flue chat or agent proxy. Keep the Flue environment variables and `@flue/react`/`@flue/sdk` web dependencies while this categorization trace exists.
