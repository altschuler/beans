...
Category and group management writes share one domain implementation in `@penge/domain/category-management`. The web Categories page calls it through Zero mutators.

The shared domain logic trims names, checks team membership, validates category type, verifies group/account ownership, rejects locked rows, and enforces deletion rules from persisted ledger data.
