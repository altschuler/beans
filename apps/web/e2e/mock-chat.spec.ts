...
test('signed-in Zero app streams mock chat, resumes history, and isolates another user', async ({page, browser, baseURL}) => {
  ...
  await page.getByRole('button', {name: 'Mock chat', exact: true}).click()
  ...
  await expect(page.getByRole('status')).toHaveText('Running mock sandbox check…')
  ...
  try {
    ...
    await otherPage.getByRole('button', {name: 'Mock chat', exact: true}).click()
