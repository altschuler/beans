...
export type SanitizedChatEvent = { event: Record<string, unknown> };
...
export async function sanitizeChatEvent(
  raw: unknown,
  options: SanitizeOptions,
): Promise<SanitizedChatEvent> {
  return sanitizeChatEventValue(raw, options);
}
...
async function sanitizeChatEventValue(
  raw: unknown,
  options: SanitizeOptions,
): Promise<SanitizedChatEvent> {
  const source = record(raw);
  const meta = safeMeta(source?.meta);
  const noOp = (): SanitizedChatEvent => ({
    event: withMeta({ type: "session.started", data: {} }, meta),
  });
  ...
  if (source.type === "session.completed") {
    return { event: withMeta({ type: "session.completed" }, meta) };
  }
  ...
  if (source.type === "session.failed") {
    ...
    return {
      event: withMeta(
        {
          type: "session.failed",
          data: { ...error, sessionId: "redacted-session" },
        },
        meta,
      ),
    };
  ...
  if (source.type === "turn.started" || source.type === "turn.completed") {
    ...
    return turn
      ? { event: withMeta({ type: source.type, data: turn }, meta) }
      : noOp();
  }
  if (source.type === "message.received") {
    ...
    return {
      event: withMeta(
        { type: source.type, data: { ...turn, message: data.message } },
        meta,
      ),
    };
  }
  if (source.type === "step.started") {
    ...
    return step
      ? { event: withMeta({ type: source.type, data: step }, meta) }
      : noOp();
  }
  if (source.type === "step.completed") {
    ...
    if (
      ...
      !finishReasons.has(data.finishReason)
    )
      return noOp();
    return {
      event: withMeta(
        {
          type: source.type,
          data: { ...step, finishReason: data.finishReason },
        },
        meta,
      ),
    };
      ...
      event: withMeta(
        {
          ...
          data: {
            ...step,
            messageDelta: data.messageDelta,
            messageSoFar: data.messageSoFar,
          },
        },
        meta,
      ),
  ...
  if (source.type === "input.requested") {
    ...
    const resolveProposal =
      ...
      ((input) => resolveChatApprovalProposal(db, input));
    const requests = [];
    for (const rawRequest of data.requests) {
      const request = record(rawRequest);
      const action = record(request?.action);
      ...
      if (approvalToolNames.has(toolName)) {
        try {
          resolved = await resolveProposal({
            scope: options.scope,
            toolName,
            input: action.input,
          });
      ...
      const safeProposal =
        resolved.status === "ready" ? resolved.proposal : null;
      requests.push({
        requestId,
        ...
        action: {
          ...
          callId,
          toolName,
          input: safeProposal ?? { proposalUnavailable: true },
        },
      });
    }
    return {
  ...
  if (source.type === "action.result") {
    ...
    if (
      ...
      !actionStatuses.has(data.status)
    )
      return noOp();
  ...
  if (source.type === "step.failed") {
    ...
    return {
      event: withMeta(
        { type: source.type, data: { ...step, ...getSafeChatError(data) } },
        meta,
      ),
    };
  }
  if (source.type === "turn.failed") {
    ...
    return {
      event: withMeta(
        { type: source.type, data: { ...turn, ...getSafeChatError(data) } },
        meta,
      ),
    };
  }
...
function sanitizeActionRequest(value: unknown) {
  ...
  const callId = safeId(action?.callId);
  if (!action || callId === null || typeof action.kind !== "string")
    return null;
  ...
  if (action.kind === "load-skill")
    return { kind: "load-skill", callId, input: {} };
...
function sanitizeActionResult(
  ...
  if (result.kind === "tool-result")
    return {
      kind: result.kind,
      ...common,
      toolName: safeToolName(result.toolName),
    };
  if (result.kind === "load-skill-result")
    return { kind: result.kind, ...common, name: "redacted" };
  if (result.kind === "subagent-result")
    return { kind: result.kind, ...common, subagentName: "redacted" };
  return null;
}
...
function safeToolName(value: unknown) {
  return typeof value === "string" && safeToolNames.has(value)
    ? value
    : "unsupported";
}
