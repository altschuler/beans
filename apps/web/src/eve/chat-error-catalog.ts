...
export const safeChatErrorCodeSchema = z.enum(
  Object.keys(safeChatErrorCatalog) as [
    keyof typeof safeChatErrorCatalog,
    ...(keyof typeof safeChatErrorCatalog)[],
  ],
);
export type SafeChatErrorCode = z.infer<typeof safeChatErrorCodeSchema>;

export const safeChatErrorSchema = z
  .object({
  ...
  .superRefine((error, ctx) => {
    if (error.message !== safeChatErrorCatalog[error.code]) {
      ctx.addIssue({
        code: "custom",
        path: ["message"],
        message: "Safe chat error message must match the fixed catalog",
      });
    }
  });
...
export function getSafeChatError(
  raw: unknown,
  fallback: SafeChatErrorCode = "CHAT_TURN_FAILED",
): SafeChatError {
