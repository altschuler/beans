  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 lg:flex-row">
      <div
        className={cn(
          "min-w-0",
          isOpen ? "hidden lg:block lg:flex-1" : "block",
        )}
      >
        {trigger}
      </div>
...
export function TeamChatPanel({
  ...
  const [selectedChat, setSelectedChat] = useState<SelectedTeamChat | null>(
    null,
  );
  const titleId = useId();
  const zero = useZero();
  const query =
    teamId && userId
      ...
      : queries.domain.teamDataAssistantChatsByTeamUser({
          teamId: pendingChatScopeSentinel,
          userId: pendingChatScopeSentinel,
        });
  ...
  const createAndSelectNewChat = useCallback(
    (scope: { teamId: string; userId: string }) => {
      ...
      void runZeroMutation(
        zero.mutate(mutators.assistant.createTeamDataAssistantChat(chat)),
  ...
  const touchChat = useCallback(
    (chatId: string, submitted = false) => {
      const now = Date.now();
      void runZeroMutation(
        zero.mutate(
          mutators.assistant.touchTeamDataAssistantChat({
            chatId,
            lastUsedAt: now,
  ...
  useEffect(() => {
    ...
    if (selectedChat?.teamId === teamId && selectedChat.userId === userId)
      return;
    ...
    if (
      ...
      Date.now() - latestChat.lastUsedAt < recentChatResumeWindowMs
    ) {
      setSelectedChat({
        id: latestChat.id,
        teamId,
  ...
  useEffect(() => {
    ...
    const focusTimer = setTimeout(() => {
      ...
      const target =
        ...
        panel?.querySelector<HTMLElement>('[aria-label="Close chat"]');
      target?.focus();
    }, 0);
    return () => {
  ...
  function selectChat(chat: SelectedTeamChat) {
    setSelectedChat({ ...chat, waitForChatCreation: false });
    setShowHistory(false);
    touchChat(chat.id);
  }
  ...
  return (
    <aside
      ...
      data-slot="team-chat-panel"
      className={cn(
        "flex h-full min-h-0 w-full flex-col border-t bg-background lg:w-96 lg:shrink-0 lg:border-t-0 lg:border-l",
        className,
      )}
    >
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              disabled={submittedChats.length === 0}
      ...
      {showHistory ? (
        <div className="border-b bg-background p-3">
          ...
          <div
            role="list"
            aria-label="Ask Penge chat history"
            className="space-y-1"
          >
            {submittedChats.map((chat) => (
              <div key={chat.id} role="listitem">
                <button
                  type="button"
                  aria-current={
                    selectedChat?.id === chat.id ? "true" : undefined
                  }
                  className={cn(
      ...
      {selectedChat ? (
        <EveChatSession
          ...
          waitForChatCreation={selectedChat.waitForChatCreation}
          onFirstSubmit={() => touchChat(selectedChat.id, true)}
        />
      ) : (
